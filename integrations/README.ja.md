...
先にターミナルで `ssh my-server` が鍵認証で成功することを確認し、設定画面へ `my-server` を入力して「設定する」を押してください。ローカルのSSH設定をバックアップしてリバースフォワードを追加し、SSH先のCodex・Claude Code通知フックを既存設定を残したまま追加します。SSH先には `python3` が必要です。
